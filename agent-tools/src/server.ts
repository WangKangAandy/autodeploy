...
/**
 * Main MCP Server for Agent Tools
 */
class RemoteToolsServer {
  ...
  async start() {
    ...
    console.error("[Agent Tools MCP Server] Started successfully")
  }

  async stop() {
    await this.server.close();
    console.error("[Agent Tools MCP Server] Stopped")
  }
}
...
process.on("SIGINT", async () => {
  console.error("\n[Agent Tools MCP Server] Received SIGINT, shutting down...")
  await server.stop()
  process.exit(0)
})

process.on("SIGTERM", async () => {
  console.error("\n[Agent Tools MCP Server] Received SIGTERM, shutting down...")
  await server.stop()
  process.exit(0)
})
...
server.start().catch((error) => {
  console.error("[Agent Tools MCP Server] Failed to start:", error)
  process.exit(1)
})